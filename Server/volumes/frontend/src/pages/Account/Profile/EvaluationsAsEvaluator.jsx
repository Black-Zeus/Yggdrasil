...
const EvaluationsAsEvaluator = ({ evaluations, onAction }) => {
  ...
  if (!evaluations || evaluations.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-subtle mb-6 p-6">
        ...
        <p className="text-text-muted py-4 text-center">No hay evaluaciones pendientes como evaluador.</p>
      </div>
    );
  }
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-highlight">
                ...
                <th className="py-3 px-4 text-left font-medium text-primary">Evaluado</th>
            ...
            <tbody>
              {evaluations.map((evaluation) => (
                <tr key={evaluation.id} className="border-b border-border-light">
                  ...
                  <td className="py-3 px-4">{evaluation.evaluatedPerson}</td>
...
export default EvaluationsAsEvaluator;